...
export const ProductCard: React.FC<ProductCardProps> = ({ product, index }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center pointer-events-none sm:pointer-events-auto">
          <button
            ...
            className="px-4 py-2.5 bg-white text-black font-head text-[11px] font-bold tracking-widest uppercase flex items-center gap-2 hover:bg-white/90 shadow-xl transition-transform transform translate-y-2 group-hover:translate-y-0 duration-300"
          >
      ...
      <div className="p-4 sm:p-5 flex flex-col justify-between flex-1 border-t border-white/10 bg-[#0e0e0e] space-y-4">
        <div>
          <div className="flex items-start justify-between gap-2">
            <h3
              onClick={() => openProductModal(product)}
              className="font-head text-base sm:text-lg font-bold tracking-tight text-white uppercase hover:text-white/80 transition-colors cursor-pointer"
            >
              {product.name}
            </h3>
        ...
        <div className="space-y-2 pt-2 border-t border-white/5">
          ...
          <div className="flex flex-wrap gap-1">
            {product.sizes.map((size) => (
              <button
                ...
                className={`min-w-[28px] h-6 px-1.5 text-[10px] font-mono font-semibold flex items-center justify-center border transition-all ${
                  selectedSize === size
          ...
          <div className="pt-2 flex gap-2">
            <button
              ...
              className="flex-1 py-2.5 bg-white/10 hover:bg-white text-white hover:text-black font-head text-[11px] font-bold tracking-widest uppercase border border-white/20 hover:border-white transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
            ...
            <button
              onClick={() => openProductModal(product)}
              className="p-2.5 border border-white/15 hover:border-white text-white/70 hover:text-white transition-colors flex items-center justify-center"
              title="Inspect Piece"
            >
